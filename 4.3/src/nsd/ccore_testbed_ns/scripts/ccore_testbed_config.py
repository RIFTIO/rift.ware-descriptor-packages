...
def configure_sipp(logger, run_dir, vnf_mgmt_ip, vm_mgmt_ip, dns_mgmt_ip, vnfr_name):
    ...
    sh_file = "{}/configure_sipp-{}.sh".format(run_dir, time.strftime("%Y%m%d%H%M%S"))
...
send "cp /tmp/data1.csv data1.csv\r"
expect "]$ "
send "cp /tmp/reg_auth_dereg.xml reg_auth_dereg.xml\r"
expect "]$ "
#5551239800;[authentication username=5551239800@rift.io password=mypass0]
send "sed -i \"s/5551239800/1001/g\" data1.csv\r"
expect "]$ "
send "sed -i \"s/rift.io/test.com/\" data1.csv\r"
expect "]$ "
send "sed -i \"s/rift.io/test.com/\" data1.csv\r"
expect "]$ "
send "sed -i \"s/password=mypass0/password=Test123/\" data1.csv\r"
expect "]$ "
...
# Rename Host
set sippname {vnfr_name}
set sippname2  [string map {{"_" ""}} $sippname]
send "echo $sippname2.test.com > /etc/hostname\r"
expect "]# "
send "hostname -F /etc/hostname\r"
expect "]# "

send "yum install -y bind-utils\r"
expect "]# "
# Point to local DNS server
send "echo 'nameserver {dns_mgmt_ip}' > /etc/resolv.conf\r"
expect "]# "
exp_close -i $spid
'''.format(vnf_mgmt_ip=vnf_mgmt_ip, vm_mgmt_ip=vm_mgmt_ip, dns_mgmt_ip=dns_mgmt_ip, vnfr_name=vnfr_name))
