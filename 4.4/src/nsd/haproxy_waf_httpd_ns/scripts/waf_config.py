...
send "echo '$date Added WAF {waf_server_name} {waf_cp_ip}' >> /tmp/progress\r"
expect "]# "
...
set date [clock format [clock seconds] -format {{%Y-%m-%d %k:%M:%S}}]
send "echo '$date Removed WAF {waf_server_name} {waf_cp_ip}' >> /tmp/progress\r"
expect "]# "
