...
def copy_file_ssh_sftp_data(logger, server, username, dirname, filename, data):
    ...
    filehdl.close()
    sshclient.close()
    logger.debug("Done with copying file")
...
'''.encode('ascii')
    ...
    copy_file_ssh_sftp_data(logger, vnf_mgmt_ip, 'fedora', '/tmp/', 'test.com.conf', file_str)
...
expect "]# "
sleep 2
send "echo 1 > /tmp/progress\r"
expect "]# "

send "sed -iE 's/# interface: 0\.0\.0\.0/interface: 0\.0\.0\.0/' /etc/unbound/unbound.conf\r"
expect "]# "
send "echo 2 >> /tmp/progress\r"
expect "]# "
send "sed -iE 's/# access-control: 0\.0\.0\.0\\/0 refuse/access-control: 0\.0\.0\.0\\/0 allow/' /etc/unbound/unbound.conf\r"
expect "]# "
send "echo 3 >> /tmp/progress\r"
expect "]# "
send "sed -iE 's/interface-automatic: no/interface-automatic: yes/' /etc/unbound/unbound.conf\r"
expect "]# "
send "echo 4 >> /tmp/progress\r"
expect "]# "
send "cp /tmp/test.com.conf /etc/unbound/local.d/test.com.conf\r"
expect "]# "
sleep 2
send "echo 5 >> /tmp/progress\r"
expect "]# "
sleep 2
exp_close -i $spid
...
def main(argv=sys.argv[1:]):
    ...
    try:
        ...
        logger.debug("Done with configuring DNS server VNF..")
